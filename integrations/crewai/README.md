...
tool reports which effect was refused. `VelarisRunTool()` with no
`allow` is the same `io`, which is also what `velaris.run` and the
command line give a run that asks for nothing (velaris-lang 5.0).
